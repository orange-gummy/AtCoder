...
int main() {
  ...
  //code start
  int n;cin>>n;
  set<tuple<ll,ll,ll>> A;
  for(int i=1;i<pow(n,1/3);++i)for(int j=i;j<pow(n/i,1/2);++j){
    for(int k=1;i<=n/(i*j);++k){
      if(k>=i&&k>=j)A.insert(make_tuple(i,j,k));
    }
  }
  cout<<size(A)<<endl;
  //code end
