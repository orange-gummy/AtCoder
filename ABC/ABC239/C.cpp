...
int main() {
  ...
  vpll A{mp(a+1,b+2),mp(a+2,b+1),mp(a-1,b+2),mp(a-2,b+1),mp(a-1,b-2),mp(a-2,b-1),mp(a+1,b-2),mp(a+2,b-1)};
  vpll B{mp(aa+1,bb+2),mp(aa+2,bb+1),mp(aa-1,bb+2),mp(aa-2,bb+1),mp(aa-1,bb-2),mp(aa-2,bb-1),mp(aa+1,bb-2),mp(aa+2,bb-1)};
  rep(i,8)rep(j,8){
    if((A[i].F==B[j].F)&&(A[i].S==B[j].S)){cout<<"Yes"<<endl;return 0;}
  }
  cout<<"No"<<endl;
  //code end
  return 0;
