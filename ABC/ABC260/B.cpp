...
int main() {
  ...
  rep(ii,x){
    ...
    int now=-1;
    rep(i,n){
      if(B[i]!=true&&A[i].F>max){max=A[i].F;now=i;}
    }
    B[now]=true;
  }
  rep(ii,y){
    ...
    int now=-1;
    rep(i,n){
      if(B[i]!=true&&A[i].S>max){max=A[i].S;now=i;}
    }
    B[now]=true;
  }
  rep(i,z){
    ...
    int now=-1;
    rep(j,n){
      if(B[j]!=true&&A[j].S+A[j].F>max){max=A[j].S+A[j].F;now=j;}
    }
    B[now]=true;
  }
  rep(i,n)if(B[i]==true)cout<<i+1<<endl;
  //code end
  return 0;
}