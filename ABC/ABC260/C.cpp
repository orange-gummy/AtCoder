...
ll n,x,y;
vpii b;
ll redblue(bool w,ll sum,ll level){
  if(w){if(level<2)return 0;
  else return redblue(true,sum,level-1)+redblue(false,sum*x,level);}
  else{
      sif(level==1)return sum;
  else return redblue(true,sum,level-1)+redblue(false,sum*y,level-1);
  }
}
int main() {
