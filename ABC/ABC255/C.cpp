...
int main() {
  ...
  //code start
  ll x,a,d,n;cin>>x>>a>>d>>n;--n;
  if(d==0){
    cout<<abs(x-a)<<endl;return 0;
  }
  if(d<0){
    if(x>a){
      cout<<x-a<<endl;return 0;
    }else if(x<a+d*n){
      cout<<(a+d*n)-x<<endl;return 0;
    }
  }else{
    if(x<a){
      cout<<a-x<<endl;return 0;
    }else if(x>a+d*n){
      cout<<x-(a+d*n)<<endl;return 0;
    }
  }
  cout<<min(abs((x-a)%d),abs(d-(x-a)%d))<<endl;
  //code end
  return 0;
}