...
int main() {
  ...
  vpll B(n);rep(i,n)cin>>B[i].F>>B[i].S;
  rep(i,n){
    ll a=LONG_LONG_MAX;
    rep(j,k){
      a=min(a,(B[i].F-B[A[j]-1].F)*(B[i].F-B[A[j]-1].F)+(B[i].S-B[A[j]-1].S)*(B[i].S-B[A[j]-1].S));
    }
    if(a!=LONG_LONG_MAX)ans=max(ans,a);
    
  }
  cout << std::fixed << std::setprecision(15) << sqrt((double)ans)<<endl;
  //code end
  return 0;
}