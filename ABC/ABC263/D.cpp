...
int main() {
  ...
  ll n,l,r;cin>>n>>l>>r;
  ll ans=0;
  vll A(n);rep(i,n){cin>>A[i];ans+=A[i];}
  vll B(n);vll C(n);
  ll x=-1;
  rep(i,n){
    if(i==0)B[i]=ans-A[i]+l;
    else B[i]=B[i-1]-A[i]+l;
  }
  if(*min_element(all(B))<=ans){x=distance(B.begin(),vecmin(B));}
  ans=min(ans,*vecmin(B));
  rep(i,x+1)A[i]=l;
  rep(i,n){
    if(i==0)C[0]=ans-A[n-1]+r;
    else C[i]=C[i-1]-A[n-1-i]+r;
  }
  cout<<min(ans,*vecmin(C),*vecmin)<<endl;
  //code end
  return 0;
}