...
int main() {
  ...
  int now=0;
  while(true){
    while(A[n-1-now]!=m-now){if(now==n)break;++now;}
    if(now=m)break;
    ++A[n-1-now];rep(i,now)A[n-now+i]=A[n-now+i-1]+1;
    now=0;
    if(A[n-1]>m)break;
    else{
      rep(i,n){if(i!=n-1)cout<<A[i]<<" ";else cout<<A[i]<<endl;}
    }
  }
  //code end
  return 0;
