...
int main() {
  ...
  int n,k;cin>>n>>k;
  vi A(n),B(n);
  vector<pair<bool,bool>> X(n,mp(false,false));
  rep(i,n)cin>>A[i];
  rep(i,n)cin>>B[i];
  rep(i,n){
    if(i==0){
      X[i].F=true;X[i].S=true;
    }else{
      if(X[i-1].F){
        if(abs(A[i-1]-A[i])<=k)X[i].F=true;
        if(abs(A[i-1]-B[i])<=k)X[i].S=true;
      }
      if(X[i-1].S){
        if(abs(B[i-1]-A[i])<=k)X[i].F=true;
        if(abs(B[i-1]-B[i])<=k)X[i].S=true;
      }
      if(X[i].F==false&&X[i].S==false){
