...
#define REPS(i, a, b) for(ll i = (b)-1;i >= (a);--i)
...
int main() {
  ...
  rep(i,n-1){
    ll a;cin>>a;
    if(A[i]+sum[i%3]+a-back<0)sum[i%3]+=A[i]+sum[i%3]-a+back;
    A[i+3]=A[i]+a-back;
    back=a;
  }
  if(sum[0]+sum[1]+sum[2]>first){cout<<"No"<<endl;return 0;}
  sum[0]+=first-(sum[0]+sum[1]+sum[2]);
  cout<<"Yes"<<endl;
  rep(i,n+2){
    cout<<A[i]+sum[i%3];
    if(i!=n+1)cout<<" ";
