...
int main() {
  ...
  vi A(h);vi B(w);
  int H=0;int W=0;
  rep(i,h){cin>>A[i];H+=A[i];}
  rep(i,w){cin>>B[i];W+=B[i];}
  if(H%k!=W%k){cout<<-1<<endl;return 0;}
  int ans=0;
  rep(i,w){
    ans+=((((k-1)*h)-B[i])/k)*k+B[i];
  }
  int anss=0;
  rep(i,h){
    anss+=((((k-1)*w)-A[i])/k)*k+A[i];
  }
  cout<<min(ans,anss)-k-k<<endl;
  //code end
  return 0;
}