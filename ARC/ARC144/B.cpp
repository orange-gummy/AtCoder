...
ll n,a,b;
bool isOK(ll index, vll key) {
  ll okk=0;
    rep(i,n){
      if(key[i]<index)okk+=(index-key[i]-1)/a+1;
      else okk-=(key[i]-index)/b;
    }
    if(okk>0)return false;
    else return true;
}

ll binary_search(vll key) {
    ll ng = pow(10,9)+1;
    ll ok = -1;
