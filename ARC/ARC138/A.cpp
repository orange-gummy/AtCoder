...
bool cmp(pair<int, int> const& n, pair<int, int> const& p)
{
    // For instance...
    return (p.first > n.F);
}
...
int main() {
  ...
  sort(rall(B));
  rep(i,n-k)if(i!=0)B[i].S=min(B[i-1].S,B[i].S);
  int ans=INT_MAX;
  int b=0;
  while(b<k){
    auto c=upper_bound(rall(B),mp(A[k-1-b],0),cmp);
    if(c!=B.rend()){
      auto d=*c;
      ans=min(d.S+1+b,ans);
    }
